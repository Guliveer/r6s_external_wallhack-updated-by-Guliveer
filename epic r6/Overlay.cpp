...
unsigned long Color = D3DCOLOR_RGBA(255, 0, 255, 255);
unsigned long ColorHP = D3DCOLOR_RGBA(0, 255, 0, 255);
...
void Overlay::MainLoop() {
	...
	for (int i = 1/*0 is local player in situations and lw htunt, set to 0 if playing mp*/; i < 30; i++) {
		...
		if (Player.Health > 0				//Health over 0
			...
			{
				//Draw bones
				/*Head-Neck*/		  Over.draw_line(Player.w2sHead.x, Player.w2sHead.y, Player.w2sNeck.x, Player.w2sNeck.y, Color);
				/*Neck-Chest*/	      Over.draw_line(Player.w2sNeck.x, Player.w2sNeck.y, Player.w2sChest.x, Player.w2sChest.y, Color);
				/*Chest-Stomach*/     Over.draw_line(Player.w2sChest.x, Player.w2sChest.y, Player.w2sStomach.x, Player.w2sStomach.y, Color);
				/*Stomach-Pelvis*/    Over.draw_line(Player.w2sStomach.x, Player.w2sStomach.y, Player.w2sPelvis.x, Player.w2sPelvis.y, Color);
				/*Pevlis-Feet*/		  Over.draw_line(Player.w2sPelvis.x, Player.w2sPelvis.y, Player.w2sPos.x, Player.w2sPos.y, Color);
				/*Chest-RIGHT_HAND*/  Over.draw_line(Player.w2sChest.x, Player.w2sChest.y, Player.w2sRHand.x, Player.w2sRHand.y, Color);
			}
			else if (bSkl != 83 && bSkl != 115)
			{
				//Draw player box
				Over.draw_outlined_rect(Player.ScrenTop.x - BoxWidth / 2, Player.ScrenTop.y, BoxWidth, BoxHeight, Color);
			}
			
			//Draw healthbar
			Over.DrawHealthbars((Player.ScrenTop.x - BoxWidth / 2) - 8, Player.ScrenTop.y, BoxHeight, Player.Health, ColorHP);
	
			//Draw a circle around the players head
			Over.DrawCircle(Player.w2sHead.x, Player.w2sHead.y, BoxHeight / 12.5, 30, Color);

			//Draw name
			//Over.draw_text("Terrorist", Player.ScrenTop.x, Player.ScrenTop.y - 14, 0xFFFFFFFF, true, true, d3Font);

			//Draw snapline
			//Over.draw_line(screenX / 2, screenY, Player.w2sHead.x, Player.ScrenTop.y + BoxHeight, Color);
		}
	}
}
