...
void Memory::UpdateAddresses() {
	...
	pCamera = RPM<DWORD_PTR>(GetBaseAddress() + ADDRESS_GAMEPROFILE);

	pCamera = RPM<DWORD_PTR>(pCamera + GAMEPROFILE_CHAIN1);
	pCamera = RPM<DWORD_PTR>(pCamera + GAMEPROFILE_CHAIN2);
	pCamera = RPM<DWORD_PTR>(pCamera + GAMEPROFILE_CHAIN3);
}
...
Vector3 Memory::WorldToScreen(Vector3 position) {
	...
	return Vector3((displayWidth / 2) * (1 + x / GetFOVX() * -1 / z), (displayHeight / 2) * (1 - y / GetFOVY() * -1 / z), z);
}